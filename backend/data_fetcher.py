...
def clean_keyword(keyword: str) -> str:
    ...
    return keyword.lower().replace(' ', '').replace('&', '').replace('-', '').replace('.', '').replace(',', '')

def build_address(street: Optional[str], city: Optional[str], state: Optional[str], 
                  postal_code: Optional[str], country: Optional[str]) -> Optional[str]:
...
async def fetch_and_save_business_data(db: Session, keyword: str):
    try:
        ...
        async with httpx.AsyncClient(timeout=30.0) as client:
            ...
            for business in businesses:
                ...
                if date_registration:
                    date_part = date_registration[:10] if len(date_registration) >= 10 else date_registration
        ...
        businesses = db.query(BusinessResult).filter(BusinessResult.keyword.contains(keyword)).all()
        for business in businesses:
            ...
            async with httpx.AsyncClient(timeout=10.0) as client:
                ...
                try:
                    ...
                    if filings:
                        ...
                        business.last_report_filed = filing_date[:10] if filing_date else None
                except Exception as e:
